...
function [sys,x0,str,ts] = quadrotor_dynamics(t,x,u,flag, quad)
   ...
   %z0 = [-1 0 -.15];              %   z0      Position initial conditions         1x3
     ...
     %z0 = [10 10 -.15];  
     z0 = [quad.init_pos(1,1) quad.init_pos(1,2), quad.init_pos(1,3)];
     %z0 = [0 0 -0.5]; 
...
function sys = mdlDerivatives(t,x,u,quad)
    ...
    z = x(1:3);   % position in {W} - East, North Down
    n = x(4:6);   % yaw pitch roll angles {W}
    v = x(7:9);   % velocity in {W} East, North Down
    o = x(10:12); % angular velocity in {W} yaw, pitch, roll
